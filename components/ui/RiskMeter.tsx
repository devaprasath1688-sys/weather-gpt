...
export function RiskMeter({
  ...
  const { t } = useLanguage();
  let level = t("risk.low");
  let explanation = t("risk.lowDetail");
  ...
  if (score >= 80) {
    level = t("risk.severe");
    explanation = t("risk.severeDetail");
    Icon = ShieldAlert;
  ...
  return (
    <div className={`rounded-2xl border border-[#142a47] bg-[#0a1628]/90 p-5 space-y-4 shadow-lg backdrop-blur-md ${className}`}>
      ...
      <div className="wgpt-body-text pt-2.5 border-t border-[#142a47] text-xs text-slate-300">
        <span className="text-sky-300 font-semibold font-mono">{t("risk.rationale")} </span>
